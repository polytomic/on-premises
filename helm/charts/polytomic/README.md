...
| polytomic.roles.task.ephemeral_storage_maximum | int | `0` | Ephemeral (local) storage limit for executor pods (MiB). Maps to the K8s ephemeral-storage resource limit. 0 = leave unset. Raised to match ephemeral_storage_request if set below it. |
| polytomic.roles.task.ephemeral_storage_request | int | `0` | Ephemeral (local) storage request for executor pods (MiB). Maps to the K8s ephemeral-storage resource request. 0 = leave unset (cluster default). Set this on GKE Autopilot, which otherwise imposes a default 1Gi ephemeral-storage limit and evicts pods (e.g. bulk syncs) that exceed it. Inherited by the bulk/ingest/proxy/scheduler roles unless overridden. |
