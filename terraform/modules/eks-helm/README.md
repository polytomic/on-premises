...
| <a name="input_force_update"></a> [force\_update](#input\_force\_update) | Force Helm to update the release even if no changes are detected. Useful for CI/CD pipelines. | `bool` | `false` | no |
...
| <a name="input_polytomic_dd_agent_image"></a> [polytomic\_dd\_agent\_image](#input\_polytomic\_dd\_agent\_image) | Image name for the Datadog Agent DaemonSet. Defaults to polytomic-dd-agent. | `string` | `"polytomic-dd-agent"` | no |
...
| <a name="input_polytomic_use_dd_agent"></a> [polytomic\_use\_dd\_agent](#input\_polytomic\_use\_dd\_agent) | Deploy Datadog Agent DaemonSet for APM tracing. Matches ECS module variable. | `bool` | `false` | no |
