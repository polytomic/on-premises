...
locals {
  ...
  standard_env_vars = {
    ...
    DEFAULT_OPERATIONAL_BUCKET          = "s3://${var.prefix}-${var.bucket_prefix}${local.polytomic_export_bucket}?region=${var.region}",
    ...
    RECORD_LOG_BUCKET                   = "${var.prefix}-${var.bucket_prefix}${local.polytomic_export_bucket}",
